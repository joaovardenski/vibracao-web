...
export default function Dashboard() {
  ...
  const statusStyles = {
    approved: { bg: "bg-emerald-50 text-emerald-700 border-emerald-100", label: "Aprovado" },
    pending: { bg: "bg-amber-50 text-amber-700 border-amber-100", label: "Pendente" },
    expired: { bg: "bg-slate-100 text-slate-600 border-slate-200", label: "Expirado" },
    cancelled: { bg: "bg-red-50 text-red-700 border-red-100", label: "Cancelado" },
  };
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-xs flex items-center justify-between">
            <div>
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Arrecadação Total</p>
              <h3 className="text-2xl font-black text-slate-800 mt-1">
                {new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(stats.total_revenue)}
              </h3>
              <p className="text-[11px] text-emerald-600 font-semibold mt-1">Apenas pedidos aprovados</p>
            </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-xs flex items-center justify-between">
            <div>
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Vagas Restantes</p>
              <h3 className="text-2xl font-black text-slate-800 mt-1">{stats.remaining_spots} <span className="text-sm font-normal text-gray-400">/ {totalSpots}</span></h3>
              <p className="text-[11px] text-indigo-600 font-semibold mt-1">{filledSpots} inscrições confirmadas</p>
            </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-xs grid grid-cols-2 gap-4 sm:col-span-2 lg:col-span-1">
            ...
            <div className="bg-slate-50/60 p-3 rounded-xl flex items-center gap-2.5">
              ...
              <div>
                <p className="text-[10px] font-bold text-gray-400 uppercase">Cancelados</p>
                <p className="text-lg font-black text-slate-800">{stats.cancelled}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-xs overflow-hidden">
          <div className="p-6 border-b border-slate-50 flex items-center justify-between">
            ...
            <Link to={"/admin/inscritos"} className="text-xs font-semibold text-indigo-600 hover:underline cursor-pointer flex items-center gap-1">
                ...
                {stats.latest_registrations.map((reg) => (
                  <tr key={reg.id} className="hover:bg-slate-50/50 transition">
                    ...
                    <td className="py-4 px-6 font-bold text-gray-700">
                      {new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(reg.amount)}
                    </td>
                    <td className="py-4 px-6">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-1 text-xs font-bold rounded-full border ${statusStyles[reg.status].bg}`}>
            ...
            {stats.latest_registrations.map((reg) => (
              <div key={reg.id} className="p-4 space-y-3 hover:bg-slate-50/40 transition">
                <div className="flex items-start justify-between">
                  ...
                  <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 text-xs font-bold rounded-full border ${statusStyles[reg.status].bg}`}>
                    {statusStyles[reg.status].label}
                  </span>
                </div>
